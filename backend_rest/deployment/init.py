...
run_command(
    f"pip install -r requirements.txt && python manage.py collectstatic --noinput"
)
run_command(
    f"sudo supervisorctl reload && sudo supervisorctl restart {program_name} && sudo service nginx reload"
)
run_command(
    f"sudo certbot --nginx --non-interactive --agree-tos --redirect -d {domain_name}"
)
